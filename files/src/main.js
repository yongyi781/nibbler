...
if (config.disable_hw_accel) {
	try {
		...
		console.log(
			"Hardware acceleration for Nibbler (GUI, not engine) disabled by config setting."
		);
	} catch (err) {
...
const save_dialog =
	electron.dialog.showSaveDialogSync || electron.dialog.showSaveDialog;
const open_dialog =
	electron.dialog.showOpenDialogSync || electron.dialog.showOpenDialog;
...
function startup() {
	...
	win.on("close", (event) => {
		// We used to use .once() but I suppose there's a race condition if two events happen rapidly.
		...
		switch (msg.key) {
			...
			case "wdlcalibrationelo":
				set_checks(
					"Engine",
					"WDL Calibration Elo",
					msg.val === "0" ? "Use default WDL" : msg.val
				);
				break;
	...
	win.loadFile(path.join(__dirname, "nibbler.html"), { query: query });
			...
			submenu: [
				{
					...
					click: () => {
						let s = `Nibbler ${electron.app.getVersion()} in Electron ${
							process.versions.electron
						}\n\n`;
						s += `Engine: ${loaded_engine}\nWeights: ${
							loaded_weights || loaded_evalfile || "<auto>"
						}`;
						alert(win, s);
					},
				},
					...
					click: () => {
						...
						if (Array.isArray(files) && files.length > 0) {
							...
							win.webContents.send("set", {
								pgn_dialog_folder: path.dirname(file),
							});
						}
					},
				},
					...
					submenu: [
						{
							...
							type: "checkbox",
							checked:
								config.ev_pov !== "w" && config.ev_pov !== "b",
							click: () => {
								set_checks(
									"Analysis",
									"Winrate POV",
					...
					submenu: [
						{
							...
							type: "checkbox",
							checked:
								config.cp_pov !== "w" && config.cp_pov !== "b",
							click: () => {
								set_checks(
									"Analysis",
									"Centipawn POV",
					...
					submenu: [
						...
						},
						{
							...
							checked:
								...
								config.arrow_filter_value === 0.005,
							click: () => {
								set_checks(
									"Display",
									"Arrow filter (Lc0)",
						...
						},
						{
							...
							checked:
								config.arrow_filter_type === "N" &&
								config.arrow_filter_value === 0.01,
							click: () => {
								set_checks(
									"Display",
									"Arrow filter (Lc0)",
						...
						},
						{
							...
							checked:
								config.arrow_filter_type === "N" &&
								config.arrow_filter_value === 0.02,
							click: () => {
								set_checks(
									"Display",
									"Arrow filter (Lc0)",
						...
						},
						{
							...
							checked:
								config.arrow_filter_type === "N" &&
								config.arrow_filter_value === 0.03,
							click: () => {
								set_checks(
									"Display",
									"Arrow filter (Lc0)",
						...
						},
						{
							...
							checked:
								config.arrow_filter_type === "N" &&
								config.arrow_filter_value === 0.04,
							click: () => {
								set_checks(
									"Display",
									"Arrow filter (Lc0)",
				...
				},
				{
					...
					submenu: [
						...
						},
						{
							label: "Diff ≤ 0.3",
							type: "checkbox",
							checked: config.ab_filter_threshold === 0.3,
							click: () => {
								set_checks(
									"Display",
									"Arrow filter (others)",
					...
					submenu: [
						{
							...
							click: () => {
								...
								win.webContents.send("set", { graph_max: 7 });
							},
						},
						{
							label: "6",
							...
							click: () => {
								...
								win.webContents.send("set", { graph_max: 6 });
							},
						},
						{
							label: "5",
							...
							click: () => {
								...
								win.webContents.send("set", { graph_max: 5 });
							},
						},
						{
							label: "4",
							...
							click: () => {
								...
								win.webContents.send("set", { graph_max: 4 });
							},
						},
						{
							label: "3",
							...
							click: () => {
								...
								win.webContents.send("set", { graph_max: 3 });
							},
						},
						{
							label: "2",
							...
							click: () => {
								...
								win.webContents.send("set", { graph_max: 2 });
							},
						},
						{
							label: "1",
							...
							click: () => {
								...
								win.webContents.send("set", { graph_max: 1 });
							},
						},
					],
				},
			...
			submenu: [
				{
					...
					click: () => {
						...
						if (Array.isArray(files) && files.length > 0) {
							...
							if (
								file === process.argv[0] ||
								path.basename(file).includes("client")
							) {
							...
							win.webContents.send("set", {
								engine_dialog_folder: path.dirname(file),
							});
						} else {
							win.webContents.send("call", "send_ack_engine"); // Force an ack IPC to fix our menu check state.
						}
				...
				},
				{
					...
					click: () => {
						...
						if (Array.isArray(folders) && folders.length > 0) {
							...
							win.webContents.send("set", {
								syzygy_dialog_folder: path.dirname(folder),
							});
						} else {
							win.webContents.send("call", {
								fn: "send_ack_setoption",
				...
				},
				{
					...
					click: () => {
						let files = open_dialog(win, {
							...
							properties: ["openFile"],
							filters: [
								{ name: "Polyglot Files", extensions: ["bin"] },
								{ name: "All Files", extensions: ["*"] },
							],
						});
						if (Array.isArray(files) && files.length > 0) {
							...
							win.webContents.send("set", {
								book_dialog_folder: path.dirname(file),
							});
						} else {
			...
			submenu: [
				...
				},
				{
					...
					click: () => {
						...
						if (!have_warned_hw_accel_setting) {
							alert(
								win,
								"This will not take effect until you restart the GUI."
							);
				...
				},
				{
					...
					submenu: [
						...
						{
							label: translate.t(
								"Use lowerbound / upperbound info"
							),
							type: "checkbox",
	...
	setTimeout(() => {
		...
		for (let n = 0; n < items.length; n++) {
			if (items[n].checked !== undefined) {
				items[n].checked =
					items[n].label ===
					translate.t(stringify(menupath[menupath.length - 1]));
			}
		}
	}, 50);
}
