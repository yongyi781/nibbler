...
if (load_err1) {
	hub.err_receive(
		`<span class="blue">While loading config.json: ${load_err1}</span>`
	);
	hub.err_receive("");
} else if (load_err2) {
	hub.err_receive(
		`<span class="blue">While loading engines.json: ${load_err2}</span>`
	);
	hub.err_receive("");
...
promotiontable.style.left =
	(boardsquares.offsetLeft + config.square_size * 2).toString() + "px";
promotiontable.style.top =
	(boardsquares.offsetTop + config.square_size * 3.5).toString() + "px";
...
ipcRenderer.on("set", (event, msg) => {
	// Should only be for things that don't need any action except redraw.
...
let droppables = [
	// If the UI is already lagging, dropping one of these won't make it feel any worse.
	"goto_root",
	"goto_end",
	"prev",
	"next",
...
ipcRenderer.on("call", (event, msg) => {
	// Adds stuff to the queue, or drops some stuff.

	let fn;

	if (typeof msg === "string") {
		// msg is function name
		...
		// msg is object with fn and args
...
window.addEventListener("wheel", (event) => {
	...
	if (allow) {
		if (event.deltaY && event.deltaY < 0)
			input_queue.push(hub.prev.bind(hub));
		if (event.deltaY && event.deltaY > 0)
			input_queue.push(hub.next.bind(hub));
	}
});
...
window.addEventListener("dragenter", (event) => {
	// Necessary to prevent brief flashes of "not allowed" icon.
	event.preventDefault();
});

window.addEventListener("dragover", (event) => {
	// Necessary to prevent always having the "not allowed" icon.
	event.preventDefault();
});
