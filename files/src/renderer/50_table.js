...
const table_prototype = {
	...
	get_graph_y: function () {
		...
		if (this.graph_y_version === this.version) {
			...
			if (
				...
				!info.__ghost &&
				info.__touched &&
				(this.nodes > 1 || this.limit === 1)
			) {
				this.graph_y =
					info.board.active === "w" ? info.cp / 100 : -info.cp / 100;
				// let cp = info.cp;
	...
	set_terminal_info: function (reason, ev) {
...
const info_prototype = {
	...
	cp_with_pov: function (pov) {
		...
		let cp = this.cp;
		if (
			(pov === "w" && this.board.active === "b") ||
			(pov === "b" && this.board.active === "w")
		) {
			cp = -cp;
		}
		return cp / 100;
	...
	cp_string: function (pov, dp = 2) {
		...
		if (Math.abs(cp) >= 150) {
			const plies = Math.round((200 - Math.abs(cp)) * 100);
			return `${cp > 0 ? "+" : "-"}T${plies}`;
		}
	...
	stats_list: function (opts, total_nodes) {
		...
		if (this.leelaish) {
			if (typeof this.n === "number" && total_nodes) {
				...
				if (opts.n) {
					n_string += ` N: ${((100 * this.n) / total_nodes).toFixed(
						2
					)}%`;
				}
