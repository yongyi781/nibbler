...
let info_receiver_props = {
	...
	receive: function(engine, search, s) {
		...
		if (s.startsWith("info") && s.includes(" pv ") && ((!s.includes("lowerbound") && !s.includes("upperbound")) || config.accept_bounds)) {
			...
			let infovals = InfoValMany(s, ["pv", "cp", "mate", "multipv", "nodes", "nps", "time", "depth", "seldepth", "tbhits", "hashfull"]);
			...
			tmp = parseInt(infovals["hashfull"], 10);
			if (Number.isNaN(tmp) === false) {
				node.table.hashfull = tmp;				// Note this is stored in the node.table, not the move_info
			}
