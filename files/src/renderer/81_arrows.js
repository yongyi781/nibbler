...
const arrow_props = {
	draw_arrows: function(node, specific_source, next_move) {		// specific_source is a Point(), show_move is a string
		...
		if (!config.arrows_enabled || (config.hide_lines && !specific_source) || node == null || node.destroyed)
			return;
		...
		for (let info of info_list) {
			...
			let colour = info.__touched && info.subcycle >= best_info.subcycle ? config.colors[MoveQuality(best_info, info)].color : config.colors.unknown.color;
			...
			if (info.mate !== best_info.mate && Math.sign(info.mate) === Math.sign(best_info.mate)) {
		...
		for (let o of heads) {
			...
			if (o.info.__touched && o.info.subcycle >= best_info.subcycle && !config.hide_lines) {
			...
			if (specific_source || (o.info.__touched && o.info.subcycle >= best_info.subcycle)) {
				boardctx.fillStyle = o.colour;
				boardctx.beginPath();
